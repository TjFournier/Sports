...
dk_MLS_url = 'https://sportsbook.draftkings.com/leagues/soccer/usa---mls'

dk_r = requests.get(dk_MLS_url)
...
# Builds Draftkings MLS dictionary
dk_dictionary = {game: [home, draw, away] for game, home, draw, away in zip(dk_games, dk_home_odds, dk_draw_odds, dk_away_odds)}

#print(dk_dictionary)

# Bet MGM Arizona MLS data

mgm_MLS_url = 'https://sports.az.betmgm.com/en/sports/soccer-4/betting/usa-9/major-league-soccer-102849'

mgm_r = requests.get(mgm_MLS_url)
mgm_status = mgm_r.status_code
print('mgm_status=', mgm_status)
mgm_html = mgm_r.text

print(mgm_html)

mgm_soup = BeautifulSoup(mgm_html, 'html.parser')

# Pulls span tags than includes the betting odds and converts it into a string
mgm_div_tags = mgm_soup.find_all('div', class_ = 'option option-value ng-star-inserted')
mgm_str_div_tags = []
for item in mgm_div_tags:
    mgm_str_div_tags.append(str(item))

# print(mgm_str_div_tags)

# Creates a string of the current odds
mgm_odds = []
for mgm_tag in mgm_str_div_tags:
    result = re.search('>(.*?)<', mgm_tag).group(1)
    mgm_odds.append(result)
